...
uint64_t Timer::AddTask(const STimerTask& task)
{
	assert(task.fun);
	assert(run_flag_);
	...
	std::shared_ptr<STimerTaskInter> inter_task = std::make_shared< STimerTaskInter>(task, time, task_id_);
	tasks_time_id_.insert(std::make_pair(task_id_, inter_task));
	if (auto it = tasks_time_time_.find(time); it == tasks_time_time_.end())
	{
		tasks_time_time_.insert(std::make_pair(time, std::map<uint64_t, std::shared_ptr<STimerTaskInter>>({ {task_id_, inter_task} })));
	}
	else
	{
...
void Timer::Remove(uint64_t nTimeId)
{
	std::unique_lock<std::mutex> ul(tasks_mutex_);
	auto it = tasks_time_id_.find(nTimeId);
	...
	tasks_time_id_.erase(nTimeId);

	if (time_it == tasks_time_time_.end())
	{
		return;
	}
	time_it->second.erase(nTimeId);
...
void Timer::Work()
{
	EXECUTE_AT_END(work_threads_latch_->count_down(););

	std::list<std::map<uint64_t, std::shared_ptr<STimerTaskInter>>> need_execute_tasks;
...
void Timer::GainNeedExecuteTasks(std::list<std::map<uint64_t, std::shared_ptr<STimerTaskInter>>>& need_execute_tasks)
{
...
void Timer::ExecuteTasks(std::list <std::map<uint64_t, std::shared_ptr<STimerTaskInter>>>& need_execute_tasks)
{
