...
inline jaf::Coroutine<void> CoSleep(uint64_t millisecond)
{
    ...
    struct SleepAwaitable
    {
        ...
        {
            handle_ = co_handle;
            CommonTimer::Timer()->StartTask(jaf::time::STimerPara{[this]() { TimerCallback(); }, sleep_time_});
            return true;
        }
...
public:
    jaf::Coroutine<void> Sleep(uint64_t millisecond)
    {
        struct SleepAwaitable
        {
            ...
            {
                handle_ = co_handle;
                co_timer_->timer_->StartTask(jaf::time::STimerPara{[this]() { TimerCallback(); }, sleep_time_});
                return true;
            }
