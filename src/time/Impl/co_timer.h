...
inline jaf::Coroutine<void> CoSleep(uint64_t millisecond)
{
	...
	struct SleepAwaitable
	{
		...
		{
			handle_ = co_handle;
			CommonTimer::Timer()->AddTask(jaf::time::STimerTask{ [this]() { TimerCallback(); }, sleep_time_ });
			return true;
		}
...
public:
	CoTimer(std::shared_ptr<ITimer> timer = nullptr)
		: timer_(timer == nullptr ? CommonTimer::Timer() : timer_)
	{
...
public:
	jaf::Coroutine<void> Sleep(uint64_t millisecond)
	{
		struct SleepAwaitable
		{
			...
			{
				handle_ = co_handle;
				co_timer_->timer_->AddTask(jaf::time::STimerTask{ [this]() { TimerCallback(); }, sleep_time_ });
				return true;
			}
