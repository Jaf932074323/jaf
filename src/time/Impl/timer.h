...
public:
	...
	virtual uint64_t AddTask(const STimerTask& task) override;
	// 清除所有定时任务
	virtual void Clear() override;
	// 移除一个定时任务
	// nTimeId 要移除的定时任务的Id
	virtual void Remove(uint64_t task_id) override;
...
private:
	// 执行达到时间的任务
	virtual void GainNeedExecuteTasks(std::list <std::map<uint64_t, std::shared_ptr<STimerTaskInter>>>& need_execute_tasks);
	virtual void ExecuteTasks(std::list <std::map<uint64_t, std::shared_ptr<STimerTaskInter>>>& need_execute_tasks);
...
private:
	...
	std::map<uint64_t, std::shared_ptr<STimerTaskInter>> tasks_time_id_; // 作为索引的定时任务集合 key为定时任务ID
	std::map<uint64_t, std::map<uint64_t, std::shared_ptr<STimerTaskInter>>> tasks_time_time_; // 定时任务集合 第一层key为执行时间，第二层Key为定时任务ID
};
