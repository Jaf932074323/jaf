...
Iocp::Iocp(std::shared_ptr<IThreadPool> thread_pool)
    : thread_pool_(thread_pool == nullptr ? std::make_shared<SimpleThreadPool>()
                                          : thread_pool) {}
...
void Iocp::WaitEnd() { work_threads_latch_->wait(); }
...
void Iocp::WorkThreadRun() {
  ...
  while (run_flag_) {
    BOOL success = GetQueuedCompletionStatus(
        m_completionPort, &bytesTransferred, (PULONG_PTR)&completionKey,
    ...
    if (!success) {
      if (pPerIoData == nullptr) {
        ...
        std::string str =
            std::format("Iocp code error: {} \t  error-msg: {}\r\n", dw,
                        GetFormatMessage(dw));
