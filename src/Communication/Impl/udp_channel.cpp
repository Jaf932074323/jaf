...
UdpChannel::UdpChannel(HANDLE completion_handle, SOCKET socket, std::string remote_ip, uint16_t remote_port, std::string local_ip, uint16_t local_port)
...
Coroutine<bool> UdpChannel::Read(unsigned char* buff, size_t buff_size, size_t* recv_len)
{
...
Coroutine<bool> UdpChannel::WriteTo(const unsigned char* buff, size_t buff_size, std::string remote_ip, uint16_t remote_port)
{
...
bool UdpChannel::ReadAwaitable::await_suspend(std::coroutine_handle<> co_handle)
{
	...
	{
		std::string str = std::format("UdpChannel code error: {} \t  error-msg: {}\r\n", dw, GetFormatMessage(dw));
...
size_t UdpChannel::ReadAwaitable::await_resume()
{
	return recv_len_;
}
...
bool UdpChannel::WriteAwaitable::await_suspend(std::coroutine_handle<> co_handle)
{
	...
	int send = WSASendTo(socket_, &wsbuffer_, 1, nullptr, flags, (SOCKADDR*)&send_addr_, sizeof(send_addr_), &iocp_data_.overlapped, NULL);

	return true;
}

size_t UdpChannel::WriteAwaitable::await_resume()
{
	return write_len_;
}
