...
UdpChannel::UdpChannel(HANDLE completion_handle, SOCKET socket,
    ...
    : completion_handle_(completion_handle), socket_(socket), remote_ip_(remote_ip), remote_port_(remote_port), local_ip_(local_ip), local_port_(local_port)
{
...
Coroutine<SChannelResult> UdpChannel::Read(unsigned char* buff, size_t buff_size, uint64_t timeout)
{
    ...
    if (result.len == 0)
    {
        co_return SChannelResult{.success = false};
    }

    co_return SChannelResult{.success = true};
}

Coroutine<SChannelResult> UdpChannel::Write(const unsigned char* buff, size_t buff_size,
    uint64_t timeout)
{
...
Coroutine<SChannelResult> UdpChannel::WriteTo(const unsigned char* buff, size_t buff_size,
    std::string remote_ip, uint16_t remote_port,
    uint64_t timeout)
{
...
UdpChannel::ReadAwaitable::ReadAwaitable(UdpChannel* tcp_channel, SOCKET socket,
    ...
    : tcp_channel_(tcp_channel), socket_(socket), wsbuffer_{.len = (ULONG) size, .buf = (char*) buff} {}
...
UdpChannel::WriteAwaitable::WriteAwaitable(UdpChannel* tcp_channel,
    ...
    : send_addr_(send_addr), tcp_channel_(tcp_channel), socket_(socket), wsbuffer_{.len = (ULONG) size, .buf = (char*) buff}
